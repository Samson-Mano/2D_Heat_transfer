...
{
    public partial class Element_prop_form : Form
    {
        Main_form my_parent_form;
        pslg_datastructure fe_object;

        public Element_prop_form(Main_form t_form, ref pslg_datastructure t_fe_object)
        {
            InitializeComponent();
        ...
        private void Element_prop_form_Load(object sender, EventArgs e)
        {
        ...
        private void Element_prop_form_FormClosing(object sender, FormClosingEventArgs e)
        {
            static_parameters.is_childFormOpen = false; // Inform the main form that this form is closed
            static_parameters.is_elementFormOpen = false;
        ...
        private void button_ok_Click(object sender, EventArgs e)
        {
            if (radioButton_heat_source.Checked == true)
            {
                ...
                if (co_functions.Test_a_textboxvalue_validity(textBox_heat_source.Text, false, false) == true &&
                    co_functions.Test_a_textboxvalue_validity(textBox_thermal_conductivity_x.Text, false, false) == true &&
                    co_functions.Test_a_textboxvalue_validity(textBox_thermal_conductivity_y.Text, false, false) == true &&
                    co_functions.Test_a_textboxvalue_validity(textBox_element_thickness.Text, true, true) == true)
                {
                    fe_object.main_mesh.set_element_constraint(fe_object.selection_index,
                                                             co_functions.ConvertStringToDouble(textBox_heat_source.Text),
                                                             0.0f,
                                                             0.0f,
                                                             0.0f,
                                                             co_functions.ConvertStringToDouble(textBox_thermal_conductivity_x.Text),
                                                             co_functions.ConvertStringToDouble(textBox_thermal_conductivity_y.Text),
                                                             co_functions.ConvertStringToDouble(textBox_element_thickness.Text));
                }
            }
